...
const themeSelect = getId('meetonTheme');
...
function initClientPeer() {
    ...
    signalingSocket.on('peerKnocking', handlePeerKnocking);
} // end [initClientPeer]
...
function setTheme() {
    ...
    meetonTheme.selectedIndex = lsSettings.theme;
    const theme = meetonTheme.value;
    switch (theme) {
        case 'dark':
            // dark theme
...
function shareRoomByEmail() {
    Swal.fire({
        ...
        preConfirm: () => {
            ...
            const email = '';
            const emailSubject = `Please join our MeetOn P2P Video Chat Meeting`;
...
function showAbout() {
    ...
    Swal.fire({
        background: swBg,
        position: 'center',
        title: brand.about?.title && brand.about.title.trim() !== '' ? brand.about.title : 'WebRTC P2P v1.7.36',
            ...
            <div id="about">
                ...
                : `
                        ...
                        <a 
                            ...
                            href="mailto:miroslav.pejic.85@gmail.com?subject=MeetOn P2P info"> 
                            miroslav.pejic.85@gmail.com
                        </a>
                        <br /><br />
                        <hr />
                        <span>&copy; 2025 MeetOn P2P, all rights reserved</span>
                        <hr />
                        `
            }
            </div>
...
/**
 * Handle guest knocking (admission request)
 * @param {object} data
 */
function handlePeerKnocking(data) {
    const { guestId, guestName } = data;
    console.log('Guest knocking', { guestId, guestName });

    // Only logged in users (potential hosts) handle this
    // We can show a SweetAlert2 popup
    if (typeof Swal === 'undefined') return;

    Swal.fire({
        title: 'Entry Request',
        text: `Guest "${guestName}" wants to join the call.`,
        icon: 'info',
        showCancelButton: true,
        ...
        const status = result.isConfirmed ? 'admitted' : 'rejected';
        signalingSocket.emit('admissionResponse', {
            roomId: roomId,
            guestId: guestId,
            status: status
        });
